...
int main() {
	...
	do {
		printf("Client: ");
		scanf("%s", sendline);
		sendto(sockid, sendline, strlen(sendline)+1, 0, (struct sockaddr *)&ser_adr, sizeof(ser_adr));

		int n = recvfrom(sockid, recvline, sizeof(recvline), 0, (struct sockaddr *)NULL, NULL);
		recvline[n] = '\0';
		printf("Server: %s\n", recvline);
